...
#Important that you run the actigraphy workflow chunk BEFORE this one as it takes participant ID's from participants who meet the initial actigraphy screening criteria
#This is the folder with all PSG scores for participants in it, having them as .csv's is sufficient, can configure to work with server/locally
PSG_folder <- "your_folder_path"
#This is the std.dev file which contains the STV measures for all participatns who meet actigraphy criteria
id_file <- "C:/Users/sefarrell/Downloads/sleep_timing_measures/sleep_circular_stddev_HOURS.csv"
#Pull the aproppriate subjects PSG files, shaving off .csv extension
...
for (batch in batches) {
  ...
  for (file in batch) {
    ...
    #Standardize duration column as it's currently different per participant (Epoch length varies)
...
#Final clean, use this df throughout .Rmd, overwriting as we go
clean_psg_trim <- all_psg_data %>%
  ...
  group_by(Subject_ID) %>%
  mutate(#Summing duration makes it easier than keeping everything in epochs of unequal length
...
```{r Nigth PSG}
#Identify subjects whose PSG starts in the 08:00–18:00 window and exclude
...
#WASO function
```{r WASO, echo=FALSE}
#Wakening after sleep onset useful for comparing to actigraphy derived measures 
calculate_WASO <- function(df) {
  ...
  return(waso_min)  #numeric only
}
```
#Total sleep time
```{r TST, echo=FALSE}
#Total sleep time needed for exlcusion criteria
...
#Identify subjects with < 240 minutes sleep for exclusion, can change this depending on what value you want to use, if no exclusion needed just remove this section
bad_ids <- tst_table %>%
  filter(TST_min < 240) %>%
  pull(Subject_ID)
...
#Rapid eye movement sleep latency needed for exclusion criteria, can change again depending upon study length
...
rem_lat_table <- clean_psg_trim %>%
  ...
  ungroup()
print(rem_lat_table)
#To include the screening tool exlcuding participants based on their rem latency uncomment below
...
#sleep onset latency also needed forexclusion criteria
...
sol_table <- clean_psg_trim %>%
  ...
  ungroup()
print(sol_table)
#Again to exclude based on certain criteria, just uncomment code below
#weird_sol <- sol_table %>% filter(SOL_min == 0)
#clean_psg_trim <- clean_psg_trim %>%
...
#this didn't end up in pipeline but could be useful for someone
...
```{r n2}
#N2% calculation, logic very similar for other sleep stages
...
#Subjects with no N3 can be excluded by uncommenting code below
#n3_filtered <- n3_table %>%
 #filter(!is.na(N3_percent) & N3_percent != 0)
#excluded_subjects <- setdiff(unique(clean_psg_trim$Subject_ID), n3_filtered$Subject_ID)
#clean_psg_trim <- clean_psg_trim %>%
#filter(Subject_ID %in% n3_filtered$Subject_ID)
#cat(length(excluded_subjects), "excluded for no N3")
#message("clean_psg_trim now contains ", dplyr::n_distinct(clean_psg_trim$Subject_ID), " subjects.")
```
#REM% function
```{r REM%, echo=FALSE}
...
```{r sei, echo=FALSE}
#TST and TIB need to be ran before this chunk commences
calculate_sei <- function(csv_file) {
  #Calculate TST (Total Sleep Time in minutes)
  TST <- calculate_TST(csv_file)
  TIB <- calculate_tib(csv_file)
  #Handle division by zero or NA cases, shouldn't exist but just in case
...
#Filter for SEI between x and 100, set to 50 now but can change to whatever
valid_sei_subjects <- sei_table %>%
 filter(SEI >= 50, SEI <= 100) %>%
 pull(Subject_ID)
...
cat("Excluded", length(excluded_subjects), "subjects with SEI < x % or > 100%\n")
message("clean_psg_trim now contains ", n_distinct(clean_psg_trim$Subject_ID), " subjects.")
```
#Apnea checker 
```{r AHI}
#Load apnoea data - keep AHI as continuous variable in case want to use in models
#The apnoea file can be accessed from the NSRR website, it's entitled STAGESPSGKeySRBDVariables2020-08-29 Deidentified.xlsx and can be located in the section stages>files> >original>De-identified Data>PSG>SRBD Variables
apnoea_data <- read_excel("your_file_path_here") %>%
  select(Subject_ID = s_code, ahi) %>%
  distinct(Subject_ID, .keep_all = TRUE) %>%
  mutate(
    ahi_log = log(ahi + 1)  #optional log transform
  )
#Merge AHI info - keep all participants initially
clean_psg_flagged <- clean_psg_trim %>%
  select(-matches("ahi|probable_OSA|osa_category")) %>%  
  left_join(apnoea_data, by = "Subject_ID")

#Exclude participants with AHI > 30
clean_psg_flagged <- clean_psg_flagged %>%
  filter(is.na(ahi) | ahi <= 30)  #keep NA if you don't want to drop missing AHI

#Descriptive stats for AHI
cat("AHI Summary Statistics (after exclusion):\n")
print(summary(clean_psg_flagged$ahi))

#Check AHI distribution
cat("\nChecking AHI distribution...\n")
cat("AHI Range:", min(clean_psg_flagged$ahi, na.rm = TRUE), 
    "to", max(clean_psg_flagged$ahi, na.rm = TRUE), "\n")
cat("95th percentile:", quantile(clean_psg_flagged$ahi, 0.95, na.rm = TRUE), "\n")

#Continue with reduced sample
clean_psg_final <- clean_psg_flagged
cat("Total subjects retained after exclusion:", n_distinct(clean_psg_final$Subject_ID), "\n")
```
#% Main loop
...
#Build results table with PSG metrics
psg_results_table <- clean_psg_final %>%  #Use clean_psg_final (includes AHI data)
  group_by(Subject_ID) %>%
  group_modify(~ psg_metrics_for_subject(.x)) %>%
  ungroup()

#Add AHI data to the results table
psg_results_with_ahi <- psg_results_table %>%
  left_join(
    clean_psg_final %>% 
      distinct(Subject_ID, ahi), 
    by = "Subject_ID"
  )
#Be sure to run this wholly before moving to the plotting chunk
