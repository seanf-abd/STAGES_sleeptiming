...
#SJL 
```{r SJL}
#Function: convert POSIXct/character to minutes since midnight
time_to_min <- function(x) {
  #Ensure POSIXct; if already, will do nothing
...
#Circular mean of times (in minutes since midnight)
circular_mean_hours <- function(mins) {
  radians <- circular((mins %% 1440) / 1440 * 2 * pi, units = "radians")
  mean_min <- as.numeric(mean.circular(radians)) * (1440 / (2 * pi))
  #Convert back to hours (0-24)
  mean_min <- mean_min %% 1440
  return(mean_min / 60)
}

#**NEW**: Circular difference in hours, always between -12 and 12
...
#Use window_start as unique day for each sleep window
all_df <- all_df %>%
  mutate(
...
#Only include participants with exactly 13 windows (bouts)
valid_ids <- all_df %>%
  group_by(subject_id) %>%
...
if (!"n_work" %in% names(day_counts)) day_counts$n_work <- 0
if (!"n_free" %in% names(day_counts)) day_counts$n_free <- 0
day_counts <- day_counts %>%
  mutate(flag_unexpected_distribution = (n_work + n_free != 13))

#Per subject, calculate mean midsleep per day_type using circular mean
sjl_df <- filtered_df %>%
...
if (!"MS_work" %in% names(sjl_df)) sjl_df$MS_work <- NA_real_
if (!"MS_free" %in% names(sjl_df)) sjl_df$MS_free <- NA_real_

#Circular SJL calculation
sjl_df <- sjl_df %>%
...
write.csv(sjl_combined, "C:/Users/sefarrell/Downloads/social_jetlag_13window_from_outputs.csv", row.names = FALSE)
cat("SJL values saved.\n")
```
